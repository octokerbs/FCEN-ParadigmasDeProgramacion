...
foldPersonaje :: (Posicion -> String -> b) -> (b -> Direccion -> b) -> (b -> b) -> Personaje -> b 
foldPersonaje casoPersonaje casoMueve casoMuere personaje = 
    case personaje of 
        Personaje pos nombre -> casoPersonaje pos nombre 
        Mueve p dir -> casoMueve (acc p) dir
        Muere p -> casoMuere (acc p)
        where acc = foldPersonaje casoPersonaje casoMueve casoMuere

foldObjeto :: (Posicion -> String -> b) -> (b -> Personaje -> b) -> (b -> b) -> Objeto -> b 
foldObjeto casoObjeto casoTomado casoDestruido =
    case objeto of 
        Objeto pos nombre -> casoObjeto pos nombre 
        Tomado obj personaje -> caseTomado (acc obj) personaje 
        EsDestruido obj -> casoDestruido (acc obj) 
        where acc = foldObjeto casoObjeto casoTomado casoDestruido
